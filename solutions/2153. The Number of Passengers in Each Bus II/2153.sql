WITH
  ...
  BusesMetadata AS (
    SELECT
      ...
      COUNT(Passengers.passenger_id) AS count,
      ROW_NUMBER() OVER(
  ...
  Boarding AS (
    SELECT
      ...
      GREATEST(
        0,
        BusesMetadata.count - BusesMetadata.capacity
      ) AS prev_count
    FROM BusesMetadata
    WHERE `row_number` = 1
    UNION ALL
    SELECT
      ...
      LEAST(
        BusesMetadata.capacity,
        BusesMetadata.count + prev_count
      ) AS boarded,
      GREATEST(
        0,
        BusesMetadata.count + Boarding.prev_count - BusesMetadata.capacity
      ) AS prev_count
